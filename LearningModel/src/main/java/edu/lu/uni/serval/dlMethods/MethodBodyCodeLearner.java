...
public class MethodBodyCodeLearner {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    if (modelFile.exists()) {
      ...
      CNNFeatureExtractor learner = new CNNFeatureExtractor(trainingDataFile, maxSize, sizeOfEmbeddedVector, batchSize, sizeOfFeatureVector);
  ...
  static class CNNFeatureExtractor {
    ...
    public void extractFeaturesWithCNN() throws IOException, InterruptedException {
      ...
      for (int i = 0; i < nEpochs; i++) {
        Path outputFileName = Paths.get(outputPath, (i + 1) + "_" + fileName);
        int batchers = 0;
        while (trainingDataIter.hasNext()) {
          ...
          if ((batchers * batchSize) >= 100000) {
            FileHelper.outputToFile(outputFileName, features.toString(), true);
      ...
      //Save the model
      Path modelFile = Paths.get(outputPath, fileName.substring(0, fileName.lastIndexOf(".")) + ".zip");
...
//	        ModelSerializer.writeModel(model, locationToSave, saveUpdater);
      try (BufferedOutputStream stream = new BufferedOutputStream(Files.newOutputStream(modelFile.toFile().toPath()))) {
    ...
    public void extracteFeaturesWithCNNByLoadingModel() throws IOException, InterruptedException {
      ...
      Path featureFileName = Paths.get(outputPath, "TestingFeatures_" + testingFileName);
      ...
      FileHelper.outputToFile(featureFileName, featuresOfTestingData.toString(), false);
